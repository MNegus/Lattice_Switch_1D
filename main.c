...
void calculate_energy_diff(double ret_arr[2], char *output_filename, double sample_portion, int sample_regularity) {
    ...
    long arr_length =
            (long) ((double) (no_lines - start_line) / sample_regularity) + 1; // Size of array to store all the data
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < 10; i++) {
        create_energy_diff_data(&params, energy_diff_filename,
        ...
        calculate_energy_diff(ret_arr, energy_diff_filename, 0.01,
                              100); // Calculates mean and standard error of the procedure
